...
const INICIAL: EstadoTarjeta = { estado: "inicial" };
    ...
    <div className="flex flex-col gap-6">
      <form action={procesar} className="flex flex-col gap-4">
        <div className="flex flex-col gap-1.5">
          ...
          >
            Movimientos de tarjeta nacional (.xlsx)
          </label>
          <input
            id="archivo"
            name="archivo"
...
function Resumen({
  ...
  resultado: Extract<EstadoTarjeta, { estado: "ok" }>;
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="rounded-lg border border-zinc-200 bg-white px-4 py-3 dark:border-zinc-800 dark:bg-zinc-950">
        ...
        <p className="mt-0.5 text-xs text-zinc-500 dark:text-zinc-400">
          {cuotasProyectadas === 1
            ? "cuota futura proyectada"
            : "cuotas futuras proyectadas"}
        </p>
      </div>
            ...
            <Bloque
              ...
              titulo={`${desaparecidos.length} ${
                desaparecidos.length === 1
                  ? "movimiento ya no viene en el estado de cuenta"
                  : "movimientos ya no vienen en el estado de cuenta"
              }`}
            >
...
function Bloqueados({ items }: { items: CambioBloqueado[] }) {
  return (
    <Bloque
      ...
      titulo={`${items.length} ${
        items.length === 1
          ? "movimiento fue clasificado"
          : "movimientos fueron clasificados"
        ...
        >
          <p className="font-mono text-xs text-amber-700 dark:text-amber-400">
            ...
            {item.tipo === "desaparecido"
              ? "ya no viene en el estado de cuenta"
              : "cambió de monto"}
          </p>
          <p className="mt-0.5">{item.glosa}</p>
          <p className="mt-0.5 font-medium tabular-nums">
